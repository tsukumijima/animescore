...
def load_without_coconut_pair_rows(
    repo_root: Path,
...
def build_eval_utterance_rows(
    ...
    return [row for row in canonical_rows if row["shuffled_file"] in eval_audio_files]
...
def main() -> None:
    ...
    official_train_pairs, official_test_pairs = load_without_coconut_pair_rows(
        repo_root
    )
    merged_pair_rows = official_train_pairs + official_test_pairs
    grouped_utterances = load_utterance_rows(repo_root)
    canonical_rows, canonical_lookup, duplication_stats = (
        build_canonical_utterance_rows(
    ...
    summary = {
        ...
        "duplicates_with_metadata_conflict": duplication_stats[
            "duplicates_with_metadata_conflict"
        ],
